# <Graysen Schwaller>             <10/23/2023>
# <FourInSequence/ Lab section 6 / Class Section J>
# <A simple 2d list game with the goal of getting 4 in row before the opponent>
...
def printBoard(board):
    ...
    """
    os.system("cls") #STUDENT NOTE: this is kinda optional, I just didn't like the clutter
    for row in board:
        print("|",end=" ")
        for column in row:
            print(column, end=" ")
...
def checkForNextMoveWin(board, playerNumber):
    ...
    for colNum in range(len(board)):
        ...
        rowNum = getOpenRow(board,colNum)
...
def main():
    ...
    while running:
        ...
        if choice == "p":
            ...
            tab = "    "
            print("The user selects a game between one or two players.\n"+ tab +"A one player game has the user play against the computer.\n"+tab+"A two player game has two users play against one another.\n"+"Each player takes turns 'dropping' their 'pieces' into the board. \n"+tab+"The goal is for one player to connect four pieces in an unbroken sequence. \n"+tab*2+"A sequence can be horizontal, vertical, or diagonal. \n"+"The game ends when one player has... FOUR IN SEQUENCE!")
        elif choice == "q":
